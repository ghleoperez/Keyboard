...
//        self.extensionContext!.completeRequest(returningItems: [], completionHandler: nil)
        ...
        var error: NSError?
        // Check if device is capable of using biometrics
        if context.canEvaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, error: &error) {
            let reason = "Authenticate with Touch ID/Face ID to login"
            ...
            context.evaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, localizedReason: reason) {
                ...
                DispatchQueue.main.async {
                    if success {
                        ...
                        // Authentication failed, fallback to username and password login
                    }
                }
            }
        } else {
            // Device not capable of using biometrics, fallback to username and password login
        }
